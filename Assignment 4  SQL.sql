...
--Write SQL queries for the following business
questions:

--1. How many encounters did we have before the year 2020?
Select Count (*) As TotalEncounter_2000
From [dbo].[Encounters]
Where [START] < '2000-01-01'

--2.How many distinct patients did we treat beforethe year 2020?
Select Count(Distinct ID) As TotalPatientBefore2000
From [dbo].[Encounters]
Where START < '2020-01-01'


--3.How many distinct encounter classes are
documented in the encounter table?

Select Count(Distinct [ENCOUNTERCLASS]) As TotalPatientBefore2000
From [dbo].[Encounters]
Where START < '2020-01-01'

4.     
How many inpatient and ambulatory encounters did
we have before 2020?

Select[ENCOUNTERCLASS],
Count([ENCOUNTERCLASS]) AS Total_Counts
From [dbo].[Encounters]
Where START < '2020-01-01'
...
5.     
What is our patient mix by gender, race and
ethnicity?
Select *
From [dbo].[HealthcareDemoData]


Select * 
From [dbo].[FluDemoData]
where 1 = 0
...
6.     
How many states and zip codes do we treat
patients from?

Select Count(Distinct["org_state"]) As Total_States,
       Count(Distinct ["zip"]) As Total_Zip
From [dbo].[HealthcareDemoData]
...
Select  Top 1 [county], Count(*) AS Number_of_Patient
From [dbo].[Flu_DataDemo]
Group by [county]
Order By Number_of_Patient Desc


8.     
What is our patient mix for patients who had an
inpatient encounter in 2019?
...
FROM Encounters e
JOIN HealthcareDemodata H ON e.[PATIENT] = H.[patient_id]
WHERE e.ENCOUNTERCLASS = 'inpatient'
    AND e.[START] Between '2019-01-01' And '2019-12-31'
GROUP BY H.[race], H.[ethnicity]
...
9.     
How many inpatient encounters did we have in the
entire dataset where the patient was at least 21 years old at the time of the
encounter start?

SELECT COUNT(*)
FROM Encounters E
JOIN [dbo].[Flu_DataDemo] FD
  ON E.PATIENT = FD.[id]
WHERE E.ENCOUNTERCLASS = 'inpatient'
AND age >= '21'
...
10.  How
many emergency encounters did we have in 2019?
...
11.  What
conditions were treated in those encounters?

SELECT c.DESCRIPTION, COUNT(*) AS cases
FROM Conditions c
JOIN Encounters e ON c.ENCOUNTER = e.Id
WHERE e.ENCOUNTERCLASS = 'emergency'
AND YEAR(e.START) = 2019
GROUP BY c.DESCRIPTION;



12.  What
was the emergency throughput and how did that vary by condition treated?

SELECT c.DESCRIPTION,
       AVG(er.patient_waittime) AS avg_throughput_minutes
FROM HospitalER er
JOIN Conditions c ON er.patient_id = c.PATIENT
GROUP BY c.DESCRIPTION



13.  How
many emergency encounters did we have before 2020?
...
14.  Other
than nulls (where no condition was documented), which condition was most
documented for emergency encounters before 2020?

SELECT Top 1 c.DESCRIPTION, COUNT(*) AS cnt
FROM Conditions c
JOIN Encounters e ON c.ENCOUNTER = e.Id
WHERE e.ENCOUNTERCLASS = 'emergency'
...
15.  How
many conditions for emergency encounters before 2020 had average ER throughputs
above 100 minutes?
...
JOIN Conditions c ON er.patient_id = c.PATIENT
GROUP BY c.DESCRIPTION
HAVING AVG(er.patient_waittime) > 100



16.  What
is total claim cost for each encounter in 2019?

SELECT Id, Count TOTAL_CLAIM_COST
FROM Encounters
...
--17.  What
is total payer coverage for each encounter in 2019?

SELECT Id, PAYER_COVERAGE
FROM Encounters
WHERE YEAR(START) = 2019

Select Sum(PAYER_COVERAGE) As Totalpayercoverage
From [dbo].[Encounters]


--18.  Which
encounter types had the highest cost?

Select *
From [dbo].[Encounters]

SELECT ENCOUNTERCLASS, SUM(TOTAL_CLAIM_COST) AS total_cost
FROM Encounters
GROUP BY ENCOUNTERCLASS
...
19.  Which
encounter types had the highest cost covered by payers?

SELECT ENCOUNTERCLASS, SUM(PAYER_COVERAGE) AS coverage
FROM [dbo].[Encounters]
GROUP BY ENCOUNTERCLASS
...
20.  Which
payer had the highest claim coverage percentage (total payer coverage/ total
claim cost) for encounters before 2020?

SELECT payer,
       SUM(PAYER_COVERAGE)/SUM(TOTAL_CLAIM_COST) AS coverage_pct
FROM Encounters
WHERE START < '2020-01-01'
GROUP BY payer
...
---------- Forwarded message ---------
From: oloruntobi kolapo <oloruntobikolapo@gmail.com>
Date: Wed, 11 Feb 2026, 05:23
Subject: Re: Zoom Invitation from Ifelola Ajibola
To: Zoom <no-reply@zoom.us>
