...
I
Interview Prep
Data Analytics & Data Engineering & Business Intelligence Course
D
Data Engineering Course 201
ETL
E
Emade Consulting Pre-Course
PRE-COURSE
D
Data Analytics Course 101
Data Analysis
B
Business Intelligence Course 301
BI
Archived classes
Settings
Emade Subscriptions & Plans Project
**/

USE EMADE_DEV;

CREATE TABLE EMADE_PLANS (
  plan_id INTEGER,
  plan_name VARCHAR(13),
  price DECIMAL(5,2)
);

INSERT INTO EMADE_PLANS
...
DROP TABLE IF EXISTS EMADE_SUBSCRIPTIONS;
CREATE TABLE EMADE_SUBSCRIPTIONS (
  customer_id INTEGER,
  plan_id INTEGER,
  start_date DATE
);

INSERT INTO EMADE_SUBSCRIPTIONS
...
SELECT * FROM EMADE_SUBSCRIPTIONS;
...
FROM EMADE_SUBSCRIPTIONS), 1) AS Churn_Percentage 
FROM EMADE_SUBSCRIPTIONS s
JOIN EMADE_PLANS p ON p.plan_id = s.plan_id
...
ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id) 
FROM EMADE_SUBSCRIPTIONS), 0) AS Percentage FROM Ranked r
JOIN EMADE_PLANS p ON p.plan_id = r.plan_id 
JOIN EMADE_PLANS pp ON pp.plan_id = r.prev_plan_id
WHERE p.plan_name = 'churn'
AND pp.plan_name = 'trial';
...
FROM EMADE_SUBSCRIPTIONS ) SELECT p.plan_name,
COUNT(*) AS Customer_Count,
ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id)
FROM EMADE_SUBSCRIPTIONS), 1) AS Percentage FROM Ranked r 
JOIN EMADE_PLANS p ON p.plan_id = r.plan_id 
WHERE r.rn = 2 -- row 2 = plan immediately after trial 
GROUP BY p.plan_name ORDER BY Customer_Count DESC;

--7: What is the customer count and percentage breakdown of all 5 plan_name values at 2020-12-31
WITH
Latest_Plan AS ( SELECT customer_id, plan_id,
ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY start_date DESC) AS rn
FROM EMADE_SUBSCRIPTIONS WHERE start_date <= '2020-12-31' ) 
SELECT p.plan_name, 
COUNT(*) AS Customer_Count,
ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id)
FROM EMADE_SUBSCRIPTIONS WHERE start_date <= '2020-12-31'), 
1) AS Percentage FROM Latest_Plan l 
JOIN EMADE_PLANS p ON p.plan_id = l.plan_id
WHERE l.rn = 1
...
WITH
Trial_Start AS ( SELECT customer_id, start_date AS trial_date 
FROM EMADE_SUBSCRIPTIONS s
JOIN EMADE_PLANS p ON p.plan_id = s.plan_id 
WHERE p.plan_name = 'trial' ),
Annual_Start AS ( SELECT customer_id, 
start_date AS annual_date
FROM EMADE_SUBSCRIPTIONS s 
JOIN EMADE_PLANS p ON p.plan_id = s.plan_id
WHERE p.plan_name = 'pro annual' )
SELECT AVG(DATEDIFF(DAY, t.trial_date, a.annual_date)) AS Avg_Days_To_Annual
FROM Trial_Start t 
JOIN Annual_Start a ON a.customer_id = t.customer_id;
...
Days_Calc AS (
SELECT DATEDIFF(DAY, t.trial_date, a.annual_date) AS days_to_upgrade
FROM Trial_Start t
JOIN Annual_Start a ON a.customer_id = t.customer_id
)
SELECT
